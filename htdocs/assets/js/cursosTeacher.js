function buscarCursos(_idUsuario, _submit){
    ...
    type: "POST",
    url:   '/API/Cursos/api_Cursos_Teacher.php'
    
}).done(function(response){
    let respuesta=JSON.parse(response);
    let tableBody = document.getElementById("episodios");
    ...
    if(respuesta.Resultados.length>0){
        respuesta.Resultados.forEach(element => {
            ...
            row.innerHTML = `<article class="cursos episodios">
            <div class="h-thumb">
            ...
            <span class="num-episodios"> ${element.Titulo_Curso}
            </span>
            <br><h2 class="h-title">${element.Titulo_Curso}</h2>
            </header>
            <a href="/views/editCurseUI.php?idCurso=${element.idCurso}" class="lnk-blk fa-play-circle">
